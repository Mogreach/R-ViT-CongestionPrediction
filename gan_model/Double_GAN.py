...
class Generator_f(nn.Module):
    def __init__(self, input_size,out_channals):
        super(Generator_f, self).__init__()
        #输出256*256大小，5层反卷积
        strides = [1, 4, 4, 2, 2]
        padding = [0, 0, 0, 1, 1]
        channels = [input_size,512,
                    256, 128, 64, out_channals]  # 8表示8维
        kernels = [4, 4, 4, 4, 4]
        model = []
...
class Discriminator_f(nn.Module):
    def __init__(self, input_size):
        ...
        padding = [1, 1, 1, 1]
        channels = [input_size,
                    64, 128, 256,512]  # inputsize=9通道数
        kernels = [4, 4, 4, 4]
        size = [128,64,32,16]
        #主网络，将256*256卷积成16*16输出大小,输出通道512，输出层共16*16*512个参数
        model = []
        for i, stride in enumerate(strides):
            model.append(
                nn.Conv2d(
            ...
            model.append(
                nn.BatchNorm2d(channels[i + 1])
                # nn.LayerNorm(size[i],elementwise_affine=True)
            )
            model.append(
                nn.LeakyReLU(0.2)
            )
    ...
    def forward(self, x):
        x = self.main(x)#x为batch_size，512，16，16
        Dx=self.D1(x.view(x.shape[0], -1))#-1自动计算
        return Dx
    def weight_init(m):
        # weight_initialization: important for wgan
        class_name = m.__class__.__name__
