...
class One_hot():
    ...
    """
    def match(self,pixel,threasold):
    ...
    """
    def cal_cls_n_array(self,label,threasold):
        ...
        for i in range(label.shape[0]):
            for k in range(label.shape[1]):
                label_cls+=self.match(label[i][k][0],threasold)
    ...
    def main(self):
        ...
        with open("../files/train.csv") as f:
            ...
            for each in reader:
                ...
                label_cls,label_onehot=self.cal_cls_n_array(label,thresold)
                for c in label_cls:
                    cls.append(int(c))
