...
def calculated_score(threshold_idx=None,
    ...
    # f_csv.writerow([str(threshold_idx)]+[str(i) for i in [1, 2, 3, 4, 5]]) # only for gan_test
...
def multi_process_score(out_name=None, threashold=0.0, label_path=None, save_path=None):
    ...
    calculated_score_parital = partial(calculated_score, temp_path=temp_path,
                                       label_path=label_path, save_path=save_path, threshold_label=threashold,
                                       preds=preds)
    ...
    print('copying')
    # os.system('cp {} {}'.format(os.path.join(temp_path, f'{out_name}'), os.path.join(os.path.join(os.getcwd(), "/work_dir/"+save_path+'/out'), f'{out_name}')))
    os.system('mv {} {}'.format(os.path.join(temp_path, f'{out_name}'),
                                save_path))
...
def roc_prc(save_path):
    tpr_sum_List, fpr_sum_List, precision_sum_List = calculate_all(os.path.join(save_path, 'roc_prc.csv'))
    # tpr_sum_List, fpr_sum_List, precision_sum_List = calculate_all(save_path)
    ...
    y_smooth = make_interp_spline(tpr_list, p_list, k=0)(x_smooth)

    prc_numerator = 0
    for i in range(len(y_smooth) - 1):
        prc_numerator += (y_smooth[i] + y_smooth[i + 1]) * (x_smooth[i + 1] - x_smooth[i]) / 2
    os.system(f"rm -rf {os.path.join(save_path, 'test_result')}")
...
def build_metric(metric_name):
    return roc.__dict__[metric_name.lower()]


def build_roc_prc_metric(threashold=None, dataroot=None, ann_file=None, result_path=None, **kwargs):
    if ann_file:
    ...
    multi_process_score(out_name='roc_prc.csv', threashold=threashold, label_path=os.path.join(dataroot, label_name),
                        save_path=result_path)

    return roc_prc(result_path)
